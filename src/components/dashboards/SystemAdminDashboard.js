    ...
    <div className="flex h-full">
      ...
      <div className="w-56 bg-zinc-900 text-white flex-shrink-0 flex flex-col">
        <div className="px-4 py-4 border-b border-zinc-800">
          <p className="text-xs font-mono uppercase tracking-widest text-zinc-400">System Admin</p>
        </div>
      ...
      <div className="flex-1 overflow-y-auto p-6 bg-zinc-50">
        {activeTab === 'overview' && (
          <>
            <h2 className="text-lg font-bold text-zinc-900 mb-6">{t('admin_dashboard')}</h2>
                ...
                return (
                  <div key={i} className="bg-white border border-zinc-200 p-5">
                    <div className="flex items-center justify-between mb-3">
                      <p className="text-xs font-mono uppercase tracking-wider text-zinc-400">{stat.label}</p>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-0 mb-8">
              <div className="bg-white border border-zinc-200 p-5">
                <p className="text-xs font-mono uppercase tracking-wider text-zinc-400 mb-2">Platform Share (30%)</p>
                <p className="text-2xl font-extrabold text-zinc-900">KES {stats.platformShare.toLocaleString()}</p>
              </div>
              <div className="bg-white border border-zinc-200 p-5">
                <p className="text-xs font-mono uppercase tracking-wider text-zinc-400 mb-2">Operator Share (70%)</p>
                <p className="text-2xl font-extrabold text-zinc-900">KES {stats.operatorShare.toLocaleString()}</p>
              </div>
            </div>
            ...
            <div className="bg-white border border-zinc-200">
              <div className="px-5 py-4 border-b border-zinc-200">
                <p className="text-xs font-mono uppercase tracking-widest text-zinc-400">Recent Applications</p>
              </div>
                ...
                <tbody>
                  {MOCK_APPLICATIONS.map(app => (
                    <tr key={app.id} className="border-b border-zinc-50 hover:bg-zinc-50">
                      <td className="px-5 py-3 text-sm font-semibold text-zinc-900">{app.company}</td>
                      <td className="px-5 py-3 text-sm text-zinc-600">{app.name}</td>
                      <td className="px-5 py-3 text-sm text-zinc-600">{app.country}</td>
                      <td className="px-5 py-3">
                        <span className={`px-2 py-0.5 text-xs font-mono uppercase ${getStatusStyle(app.status)}`}>
                          {app.status}
                        </span>
                      </td>
                      <td className="px-5 py-3 text-sm font-mono text-zinc-600">{app.tenants}</td>
                      <td className="px-5 py-3">
        ...
        {activeTab === 'performance' && (
          <>
            <h2 className="text-lg font-bold text-zinc-900 mb-6">{t('admin_performance')}</h2>
        ...
        {activeTab === 'pnl' && (
          <>
            <h2 className="text-lg font-bold text-zinc-900 mb-6">{t('admin_pnl')}</h2>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-0 mb-8">
              {[
        ...
        {activeTab === 'settings' && (
          <>
            <h2 className="text-lg font-bold text-zinc-900 mb-6">{t('admin_settings')}</h2>
            <div className="bg-white border border-zinc-200 p-6 max-w-lg">
              <p className="text-xs font-mono uppercase tracking-wider text-zinc-400 mb-4">Mobile Money Configuration</p>
              {['M-Pesa API Key', 'M-Pesa Secret', 'Paybill Number', 'Callback URL'].map((field) => (
                <div key={field} className="mb-4">
                  ...
                  <input
                    ...
                    className="w-full px-3 py-2 border border-zinc-300 text-sm font-mono text-zinc-900 placeholder:text-zinc-400 bg-transparent focus:outline-none focus:border-zinc-600"
                  />
                </div>
              ))}
              <button className="px-6 py-2 bg-zinc-900 text-white text-xs font-bold uppercase tracking-widest border-none cursor-pointer hover:bg-zinc-800">
                {t('save')}
              </button>
            </div>
          </>
        ...
        {activeTab === 'applications' && (
          <>
            <h2 className="text-lg font-bold text-zinc-900 mb-6">{t('admin_applications')}</h2>
            <div className="bg-white border border-zinc-200">
              <table className="w-full">
                <thead>
                  <tr className="border-b border-zinc-100">
                ...
                <tbody>
                  {MOCK_APPLICATIONS.map(app => (
                    <tr key={app.id} className="border-b border-zinc-50 hover:bg-zinc-50">
                      <td className="px-5 py-3 text-sm font-semibold text-zinc-900">{app.company}</td>
                      <td className="px-5 py-3 text-sm text-zinc-600">{app.name}</td>
                      <td className="px-5 py-3 text-sm font-mono text-zinc-600">{app.email}</td>
                      <td className="px-5 py-3 text-sm text-zinc-600">{app.country}</td>
                      <td className="px-5 py-3 text-sm font-mono text-zinc-600">{app.date}</td>
                      <td className="px-5 py-3">
                        <span className={`px-2 py-0.5 text-xs font-mono uppercase ${getStatusStyle(app.status)}`}>
                          {app.status}
                        </span>
